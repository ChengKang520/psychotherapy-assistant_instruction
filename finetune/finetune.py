...
def decide_model(args, device_map):
    ...
    model_type = "Auto" if args.model_type not in ["llama", "bloom", "chatglm", "falcon", "mpt", "wizard"] else args.model_type
    ...
    elif (model_type == "falcon") or (model_type == "mpt") or (model_type == "wizard"):
    ...
    if (model_type == "chatglm"):
        ...
        model = prepare_model_for_int8_training(model)
        model = get_peft_model(model, config)
    elif (model_type == "mpt") or (model_type == "llama") or (model_type == "wizard"):
        config = LoraConfig(
            r=args.lora_r,
            lora_alpha=args.lora_alpha,
    ...
    #     args.lora_target_modules = args.lora_target_modules.replace("[", "").replace("]", "").split(",")
...
@click.option(
    ...
    default="/home/kangchen/Chatbot/Psych_BioGPT/models/input/llama/vicuna-7b-delta-v1.1",
)
@click.option("--model_type", "model_type", type=str, default="falcon")
@click.option(
    "--data_dir",
    "data_dir",
    type=str,
    default="/home/kangchen/Chatbot/chatbot-chinese/data/general/alpaca-en-zh.json",
)
@click.option(
    "--output_dir",
    "output_dir",
    type=str,
    default="/home/kangchen/Chatbot/chatbot-chinese/finetune/alpaca-7b-chinese",
)
...
@click.option("--val_set_size", "val_set_size", type=int, default=200)
